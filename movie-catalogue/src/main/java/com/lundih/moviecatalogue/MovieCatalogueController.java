...
public class MovieCatalogueController {
    ...
    public List<MovieCatalogueItem> getCatalogue(@PathVariable("userId")  String userId) {
        ...
        RatingItem[] ratings = restTemplate.getForObject("http://localhost:8082/api/v1/ratings/" + userId, RatingItem[].class);
        ...
        if (ratings != null) {
            for (RatingItem rating : ratings) {
                ...
                Movie movie =
                        restTemplate.getForObject("http://localhost:8081/api/v1/movies/" + rating.getMovieId(), Movie.class);
                catalogue.add(
